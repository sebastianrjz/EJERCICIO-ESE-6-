...
const numberFound = numbers.find((num) => num === 100)
...
const movi = movies.find((movie) => movie.date === 2010)
console.log(movi)
...
// Encontramos el alien con nombre 'Cucushumushu'
const alien = aliens.find((alien) => alien.name === 'Cucushumushu')

// Encontramos la mutación con nombre 'Porompompero'
const mutation = mutations.find((mutation) => mutation.name === 'Porompompero')

// Fusionamos ambos objetos utilizando spread operator
const merged = {
  ...alien, // Agrega todas las propiedades del alien
  mutation: { ...mutation } // Agrega la propiedad mutation con el objeto de la mutación
}
